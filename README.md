...
- **已落地**：P4 进化**三环半** ——
  ...
  事件流；三条架构守卫；右抽屉记忆页签与写入痕迹面板）**✅ 闭环**
  ② 自动捕获（会话切换异步反思 → 候选**物理隔离**在 `memory/candidates/` → 批准/拒绝/冲突标记；
  前置门 2KB；日上限与队列）**✅ 闭环**
  ③ Playbook（`evolution/playbooks/` 布局，tags 条件召回，预算与语义记忆分开；
  `save_playbook` / `recall_playbook`）**⚠️ 纯逻辑已就位但尚未接线** ——
  工具未注册进 runner、注入段未接进 system prompt、界面无面板，**模型当前调不到它**
  ④ 遗忘与度量（LRU 遗忘 + style 豁免；描述相似度警告）**✅ 闭环**；
  但**重复纠正率 / 误伤率两个指标尚无数据来源**（`correct` 与 `flag` 事件定义了但无产生入口）
