...
**v0.13.26**（分期：P0 骨架 → P1 内核 → P2 工作台 → P3 生态 → P4 进化 → P5 成熟）。

- **已完成**：P0 骨架（三进程 + 双协议 Provider + 流式对话 + Key 加密存储）· P1 内核（主循环 +
  工具层 + 三档权限门控 + 检查点回滚 + JSON 分层会话存储）· P2 工作台**接近完成**
  （工作台分栏 / Monaco 编辑与 Diff 审查 / 内置终端（真 PTY）/ 资源管理器 / 内置浏览器 /
  多会话并发 / 用量计量与省 token 档位 / 系统集成开关）
- **进行中**：P4 进化**第一批（语义记忆）已落地** —— 两条写入通路（模型 `remember` / 用户选中即记）、
  影响面三分类、注入与预算截断、事件流、三条架构守卫、右抽屉记忆页签与写入痕迹面板。
  **自动捕获（反思）与 Playbook 尚未开始**，故"会自己长经验"目前只兑现了**显式记忆**这一半
- **未开始**：P3 生态（MCP 客户端 / 规则 / 插件）· P4 其余（反思 / Playbook / 遗忘与度量）；
  P5 成熟缺端到端测试与可观测面板
