...
**v0.13.45**（分期：P0 骨架 → P1 内核 → P2 工作台 → P3 生态 → P4 进化 → P5 成熟）。
...
- **已完成**：P0 骨架（三进程 + 双协议 Provider + 流式对话 + Key 加密存储）· P1 内核（主循环 +
  ...
  任务栏自动折叠 / 电脑控制开关）
- **已落地**：P4 进化**第一批（语义记忆）** —— 两条写入通路（模型 `remember` / 用户选中即记）、
  影响面三分类、注入与预算截断、事件流、三条架构守卫、右抽屉记忆页签与写入痕迹面板
- **进行中**：P4 其余（自动捕获/反思 · Playbook · 遗忘与度量）
- **未开始**：P3 生态（MCP 客户端 / 规则 / 插件）；P5 成熟缺端到端测试与可观测面板
