 ...
 * 所以这里一律以「只有……才」开头。
 *
 * ⚠️ **2026-09-13 试过按"输出形态"分流 —— 实测证明有害，已回滚**（plan8 R9.2）：
 * 当时的假设是"输出已带报错特征时，干脆不给处方，少给一个再跑的由头"。
 * 结果在同一个 harness、同一条任务上各跑 5 次：
 *
 * | 版本 | 工具调用次数（5 次） | 中位 |
 * |---|---|---:|
 * | 通用处方（现在这个） | 3, 3, 6, 4, 10 | **4** |
 * | 形态分流 | 7, 16, 13, 9, 6 | **9** |
 *
 * **两组完全不相交** —— 不是噪声，是退化。
 * 推测机制（**未证实**，别当定论）：撤掉"怎么收窄"的指引后，模型失去一条低成本路径，
 * 只好换更多种手段反复试；而反复强调"保留区 / 原文"反倒放大了"输出被处理过"的信号。
 * **结论：处方按工具名给（通用版），不要按输出形态分流。**
 */
