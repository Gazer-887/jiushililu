...
/**
 * 一个 agent run 内**共享**的常驻 shell 槽位。
 *
 * ⚠️ 为什么需要它：`buildCommandTools` 会为每个子代理各造一份命令工具实例（确认卡要写发起者名字），
 * 而"一个实例 = 一个 shell"会让同一个 run 里出现 1+N 个常驻 shell。上限是 4 个，
 * 而 `enforceCap()` 淘汰最旧那个时**不看它是不是正在执行命令** ——
 * 结果是"用户刚点了允许的那条命令被掐掉"。共享槽位后一个 run 仍然只有一个 shell，
 * `cd` / 环境激活跨步存活的语义也与未拆实例前逐字一致。
 */
export interface ShellSlot {
  current: AgentShellSession | null
}

export function createShellSlot(): ShellSlot {
  return { current: null }
}
...
function buildSystemTools(
  ...
  /**
   * 前台命令的**持久 shell 会话**（plan28 D-085 S2）：**一个 agent run = 一个 shell**
   * （由 `ShellSlot` 承载，而不是"一个工具实例一个 shell"——同一个 run 里可能有多个命令工具实例），
   * 轮与轮之间 `cd` / `set` / 环境激活**跨步存活**；新 run 从工作区根重新开始（可预测性优先）。
   * 空闲 10 分钟自动回收（含进程树清理）、同活上限 4 个 —— 长驻泄漏由这两道闸兜住。
   *
   * plan43 S3：起壳时带上**当前运行环境**（PATH 覆盖 + 指纹）。指纹的用法见下 `getShell`。
   */
  const slot: ShellSlot = shellSlot ?? { current: null }
  const getShell = (): AgentShellSession => {
    const runtime = resolveRuntimeEnv()
    // ⚠️ 这段比对在当前设计下**不会触发** —— 因为 `runner.ts` 在 **run 开始时**求值一次，
    //    同一个 run 内 `runtime` 恒定。**它保留是防御，不是功能**：万一将来有人把
    //    `resolveRuntimeEnv` 又改回"每次现读"，这里会立刻兜住"环境变了却不换壳"的漂移。
    //    **不要把它的存在误解成"每命令都会换壳"** ——
    //    那正是 2026-09-19 复查抓到的口径错误（同一 run 内换环境 = 破坏可复现性）。
    if (slot.current && slot.current.envFingerprint !== runtime.fingerprint) {
      slot.current.dispose()
      slot.current = null
    }
    slot.current ??= createShellSession(workspaceRoot, undefined, runtime)
    return slot.current
  }
  /**
