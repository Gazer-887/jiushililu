...
// **本文件只剩装配**：解析数据目录 → 跑一次格式迁移 → 把磁盘后端接到逻辑层；六个入口的**行为全在** `conversations-core.ts`
// （不依赖 electron / fs，可单测），分层布局与原子写在 `conversations-fs.ts`（fs 可注入）。
//
// 为什么不再用 electron-store：① 要分层（正文必须搬出整表，而它只支持"整个对象一把写"）；② 要可测 —— 它在**构造函数里**
// 就锁死 `app.getPath('userData')`，只能出现在装配层、永远进不了单测链路。数据目录**惰性解析**（模块求值早于 `app.whenReady()`，
// 不在求值期取路径），顺带躲开 plan10 §2.4 P0-6 那个"'setPath' 晚于 store 构造、迁移成功却读写旧目录"的时序坑。
