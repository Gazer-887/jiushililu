...
export type ReadBackupResult =
  | {
      ...
      bytes: number
      /**
       * **这份内容不是无损读出来的**（原文不是合法 UTF-8：GBK / 二进制…）。
       *
       * 为什么这个标志能救命（独立审查实测）：退回会把**整份文本**按 UTF-8 重写回磁盘，
       * 而有损解码出来的字符再编码回去**不等于原字节** —— 实测一个 GBK 文件只改了一行，
       * 退一次之后**没被改的那几行也一起烂掉**（13 字节 → 32 字节，全变成 `efbfbd`）。
       * 这是**不可逆**的损坏，所以有损的文件一律**不给逐处退回**。
       */
      lossy: boolean
    }
...
export interface CheckpointStore {
  ...
  /**
   * 回滚**之前**，先把"即将被覆盖的当前内容"另存为一轮检查点。
   *
   * 为什么非有不可：回滚是"把现在的内容换成别的"，而**它自己不留快照**的话，
   * 用户退错了就**永远回不去** —— Agent 那一版内容只存在于磁盘上，一覆盖就没了。
   * 留了这一份，回滚就变成"可逆的动作"，与"写文件前先快照"是同一条规矩。
   *
...
export function createCheckpointStore(dir: string): CheckpointStore {
  ...
  const store: CheckpointStore = {
    ...
    readBackup(runId, rel) {
      ...
      try {
        ...
        try {
          ...
          // ⚠️ `readSync` 的返回值**必须接**（审查指出）：短读（网络盘 / 被杀软锁住）时
          //    没读满的区域留着 NUL —— 不接返回值就发现不了它，而 `applyPatch` 照样能成功，
          //    然后整体回写 → **把 NUL 写进用户的文件**。
    ...
    snapshotCurrent(runId, rel, label, owner) {
      ...
      // 用**那一轮自己记下的工作区**，而不是"当前工作区" ——
      // 用户可能已经切过工作区，拿现在这个去拼路径就会指向别处（甚至越界）。
      ...
      // ⚠️ **一条都没备份成时必须返回 null**（审查指出）：`record` 把自己的异常全吞掉
      //    （快照失败不该阻断写入 —— 那条设计是对的），但如果每条都失败了，
      //    这里仍把 preRunId 交出去，界面就会显示"已备份"、文案还在承诺
      //    "退错了还能再退" —— **兜底静默消失而界面仍在承诺它**。
      //    宁可如实说"这次没留兜底"，也不能让用户以为有。
    ...
    rollback(runId, rel) {
      ...
      /**
       * 动手之前先记下：**这一轮此刻还在不在内存登记里**。
       *
       * 用来区分两种轮次（收尾时行为不同）：
       *   · `true` = 它**正在跑** → 回滚之后必须**保持登记**，
       *     否则它后续每一次写文件都不再留快照（`record` 拿不到 run 就直接 return）
      ...
      // ⚠️ **有失败就不打这个标记**（审查指出）：整轮回滚时原写法不管 `failed` 照样标记，
      //    于是面板上同时出现「已回滚」徽章和"失败 N 个"的提示 —— 两个互相矛盾的信号，
      //    用户会以为退干净了。不打标记反而正确：他还能再点一次。
      if (report.failed.length === 0) {
        run.rolledBackAt = Date.now()
        if (run.status === 'running') run.status = 'done'
        active.set(runId, run)
        flush(run)
        // ⚠️ 只有"本来就不在 active 里"（= 崩溃残留的轮次）才摘登记。
        //    仍在跑的那一轮被摘掉的话，它**后续每一次写文件都不再进检查点**
        //    （`record` 第一句就是 `active.get(runId)`，拿不到就直接 return），
        //    而且面板里再也不会显示 —— 这是最难查的那种静默失效。
