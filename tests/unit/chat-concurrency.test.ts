 ...
 * 对话并发闸（plan11 §2.1）。
 *
 * ## 为什么这条必须单测（而不是丢给 verify-shot）
 *
 * `verify-shot` 把 `chat:send` 整个 stub 掉了 —— 渲染端自己就会把 `streaming` 置 true，
 * 所以**闸是 1 还是 3，界面上都显示两条"正在生成"**。
 * 实测：把上限改回 1 再跑，那边 6 条并发断言**照样全绿** ——
 * 也就是说那 6 条验的是"渲染端能同时持有两条"，**根本没碰到这道闸**。
 * 这类"看起来覆盖了、其实没覆盖"的差一厘米，只能靠把逻辑抽成纯函数 + 单测来补。
 */
