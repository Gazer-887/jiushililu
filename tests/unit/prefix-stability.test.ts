 ...
 * ## 为什么它值钱
 *
 * 实测（DSH 面板）：输入 1036.88M 里 **1029.66M 是缓存命中（99.3%）**。
 * 也就是说输入的钱**绝大部分本来就省下来了** —— 前提是"开头一模一样"。
 * 前缀一旦在某个点变了，从那个点往后**全部失效**，得按未命中重新计费。
 *
 * ## 所以这一组守两件事
 *
 * 1. **系统提示必须是静态的**：不许混进时间 / 轮数 / 会话 id / 随机值 ——
 *    它拼在消息最前面，是最大的一块前缀。
 * 2. **折叠历史的破坏点要知道它在哪**：`trimMessages` 会往**中间**插「[历史摘要]」，
 *    那是**已知且值得**的代价 —— 但要确保它不越界（system 仍在最前、最近几条不动）。
 */
...
describe('前缀稳定：系统提示的组成必须是静态的', () => {
  ...
  it('系统提示的拼接里**不许出现会变的东西**（时间 / 随机 / uuid）', () => {
    // 只看 `guardedSystem` 那一段构造的上下文：
    // 一旦有人往里面塞 Date.now / new Date / random / uuid，这条就红
...
/**
 * 提示措辞的守卫（2026-09-13 实测校准的直接产物）。
 *
 * 数据：一条只要求"跑一次命令"的任务，模型跑了 **9 次工具调用**，
 * 而第 1 次就拿到了完整输出（含结论）—— 它是在**反复确认**，不是没看到。
 * 定位到两处措辞在推着它多跑：
 *   ① 工具输出的通知没把"保留区是逐字节原文"说死 → 它想绕开压缩去读原始文件；
 *   ② 做事纪律第 6 条只说了"耗时的活转后台"，没提醒"**输出多 ≠ 耗时长**" →
 *      它把毫秒级的打印命令转后台，再 check_command / kill_command。
 */
