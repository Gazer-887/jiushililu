...
// 三条红线各一组用例：不许瞒（省了就必须留痕：省了多少行、行号是什么）/ 不许砍掉排错现场
//（末尾与报错行必须留）/ 压不动就原样放行（"什么都没做"是合法结果）。
...
describe('窗口化：排错现场不许被压掉', () => {
  ...
  // ⚠️ 红队给的样例，原样钉住：以后再改关键词正则，漏掉哪一类当场变红，
  //    而不是等用户排错时才发现"现场没了"。
...
describe('窗口化：双门控（压不动就原样放行）', () => {
  ...
  // ⚠️ 头尾是常数（60/40）、门是比例（0.72）→ 行数约 230 以下的中文输出无论怎么算都过不了门，
  //    且是**静默**的：功能看着做了、实际一字节没省。修法是按预算逐档收紧窗口（`scale`）。
  ...
  // ⚠️ 第二道门（token 门）**数学上被字节门蕴含、任何输入都开不了**（推理见 `tool-window.ts`
  //    的不变式注释），所以这里不断言"它能开"，只断言不变式：压过的 token 必降、放行的原文一字未动。
...
describe('窗口化：**绝对预算**（相对门控管不住"压完还是很大"）', () => {
  // ⚠️ 相对门控管不住"压完还是很大"：3MB 输入压到 72% 仍有 ~17.6k 估算 token，
  //    而超窗在厂商那边是**整轮作废**的硬错（不是多花点钱）。
  it('压完的结果**不许超过绝对预算**', () => {
  ...
  it('预算给得非常小的时候，也不是"过不了就整段放行"——仍然给最小的那档', () => {
    ...
    // ⚠️ 预算 520 而不是 400：通知文案**也占这个预算**，而通知不许省（不静默截断是红线）。
    //    断言守的是"极小预算下仍给最小档、不整段放行"——行为没变，变的是通知开销被承认进预算。
...
/**
 * 通知措辞：让模型**信**保留区 —— 实测中模型第 1 次调用就拿到了含结论的完整输出，
 * 之后仍换着法子反复确认：它不是没看到结论，是**不信"被压过的输出"**（想绕开压缩去读原始文件）。
 */
...
/**
 * 续读处方：**不许按"输出形态"分流**（实测有害，已回滚）。
 * 同一 harness、同一任务各跑 5 次：通用处方中位 4 次工具调用，形态分流中位 9 次，两组完全不相交。
 * 这条守的就是"别再来一次"：含报错特征的输出，用的仍必须是**通用处方**。
 */
