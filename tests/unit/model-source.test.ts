...
/**
 * 来源标签（plan7 F5.1 的补救）：**从 baseURL 推**，不是存一个会过期的值。
 *
 * 真事：老数据升级时把每个端点都标成了"深度求索"，连用户自己加的 agnes 端点也顶着这个标签。
 * 根因是"品牌"被当成了**要存的数据** —— 而它其实只是**地址的函数**。
 * 这一组断言钉住：推导规则稳定、认不出来一律算自定义（宁可不标，也不标错）。
 */
