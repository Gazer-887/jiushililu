...
// 工作台分栏引擎纯逻辑（plan9 W1）
//
// 这一层是地基，盯的是**边界**而非正常路径：关掉最后一个页签/栏会不会塌、
// 同一内容重复打开会不会开出重复栏、窗口窄到放不下时有没有确定的降级行为、
// 坏存档进来会不会变成半个畸形布局。
// 拖拽坐标换算**验不了真实鼠标**（Chrome 会把真实 mousemove 覆盖合成坐标）——
// 所以落点/宽度一律走纯函数，界面只验结构（本项目已验证的既定做法）。
...
describe('openTab：去重与浏览器单例', () => {
  ...
  it('文件按路径去重（点两次同一个文件不该开两个页签）', () => {
    // 用**空栏**测，页签数才等于"开过的文件数"（用 layoutWith 会混进一个 explorer 页签）
    const l = addPane(emptyLayout(), null)
    const id = l.panes[0].id
...
describe('allocate（宽度分配 —— 三级收缩）', () => {
  ...
  it('**宽工作台**下拖出来的宽度不会被缩回（真机验收抓到的回归，必留）', () => {
    // 用户机器实测（工作台 515px、两栏）：第②步坚持把末栏喂到它的 min(200)，第一栏被死锁在 315
    // —— **往右拖没反应、往左拖有用**。这条专盯那一档（窄工作台那档不冲突，验不出它）。
    const dragged = 360
...
describe('clampPaneWidth（拖拽调宽）', () => {
  ...
  it('**预算紧张时仍然拖得动**（回归：曾经上限被算成 min，怎么拖都没反应）', () => {
    // 两栏、工作台 359px：两边都到不了各自的 min(200)，但用户仍应能在 200..235 之间调配
    const w = clampPaneWidth({
      desired: [320],
      mins: [200, 200],
      count: 2,
  ...
  it('容器窄到不允许均分时**承认它**、各自保底（不再假装能均分）', () => {
    // 359px 装两栏：各 200 都放不下（400 > 355）→ 落成 [200,155]；关键是总和恰等于预算、
    // 且谁都不低于绝对下限
